...
- Tests cover contracts owned by the boundary under test; incidental absence of side effects on unmanaged files, unknown inputs, or unrelated state is not elevated into a contract without an explicit ownership model.
- Rename, deletion, and restructuring residue checks belong to implementation-time search rather than durable tests; unless explicit requirements state that former names or removed surfaces must still be handled, rejected, or validated, they are treated as obsolete and must not appear in the ongoing test contract.
