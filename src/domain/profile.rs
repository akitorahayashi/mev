...
pub fn all_profiles() -> &'static [Profile] {
    &[Profile::Default, Profile::Macbook, Profile::MacMini]
}
...
/// Validate any profile including `default` (required for `make`).
...
mod tests {
    ...
    fn resolves_canonical_profiles() {
        assert_eq!(resolve_profile("default"), Some(Profile::Default));
    ...
    fn resolves_aliases() {
        ...
        assert_eq!(resolve_profile("def"), Some(Profile::Default));
    }
    ...
    #[test]
    fn validate_machine_profile_rejects_default() {
        assert!(validate_machine_profile("default").is_err());
    }
