#!/bin/bash
# mev-internal doesn't have a binary!
# Oh, mev-internal's Cargo.toml doesn't have a [[bin]]. We need to run the CLI programmatically from tests or add a dummy [[bin]] or something. Or we just test it programmatically!
...
fn test_gh_labels_deploy() {
    ...
    assert!(log_content.contains("label delete bug"));
